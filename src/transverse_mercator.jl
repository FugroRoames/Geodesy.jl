...
function transverse_mercator(ref::LatLon, latlon::LatLon, ellipsoid::Ellipsoid, scale::Union{Type{Val{true}}, Type{Val{false}}} = Val{false})
    lat0 = ref.lat
    lon0 = ref.lon
    lat = latlon.lat
    lon = latlon.lon
...
function  tranmerc_inv(lat0, lon0, x, y, ellipsoid)
    Z = 0.0
    maxpow = 6
